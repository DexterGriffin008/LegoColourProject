...
class QuizEngine:
    ...
    def new_question(self):
        ...
        correct_rgb = hex_to_rgb_tuple(correct_row["rgb"])

        # Compute distances to all other colours
        pool = self.df[self.df["name"] != correct_name].copy()
        pool["dist"] = pool["rgb"].apply(lambda x: rgb_distance(correct_rgb, hex_to_rgb_tuple(x)))

        # Pick the closest N distractors
        distractors = pool.nsmallest(self.num_choices - 1, "dist")

        options = [correct_name] + distractors["name"].tolist()
        random.shuffle(options)

        return {
            "swatch_rgb": correct_row["rgb"],
